...
CONSENSUS_FILTER_PROMPT = """你需要根据最终共识来筛选角色的记忆。
...
## 要求：
判断这条角色记忆是否完全可以从共识中推导出来。

1. 如果共识完全覆盖了这条记忆（从共识中完全可以推导出这条记忆的全部信息，没有独特信息），返回：{{"action": "remove"}}
2. 如果这条记忆包含共识未覆盖的任何信息（独特视角、私人想法、共识中没有的细节），返回：{{"action": "keep"}}

...
参与过对话的角色被选中的可能性应该较小。禁止连续选择同一个角色两次，除非当前只有一个角色可用。
...
CONSENSUS_EXTRACT_PROMPT = """你是一个负责从多个角色的近期记忆中提取共同知识的助手。

## 各角色的近期记忆：
{all_new_memories}

## 要求：
首先，请仔细梳理事件的时间先后顺序和因果逻辑：明确哪些事情先发生、哪些后发生、事件之间的因果关系是什么。这是为了确保你提取的共识在逻辑和时序上是正确的。

然后，基于你的分析，按照以下规则提取共识：
1. 提取多个角色都会认同的关键共同事实、事件和知识。
2. 如果不同角色的记忆之间存在矛盾，不要包含矛盾的信息。只提取各角色之间一致的内容（取信息的交集）。
3. 每条共识应该是一个简洁、独立的事实陈述，需保留正确的时间和因果关系。
4. 关注共享的事件、行动和结果，而非个人想法或私人感知。

以JSON列表格式返回，每个元素为一条共识陈述字符串。示例：
["陈述1", "陈述2", ...]

只返回JSON列表，能够被json.loads()解析。
"""

CONSENSUS_DEDUP_PROMPT = """你需要判断一条新的共识记忆是否值得添加到世界全局记忆中。
...
1. 如果新条目的所有信息已经被已有记忆完全提及/覆盖，返回：{{"action": "discard", "reason": "redundant"}}
2. 如果新条目与任何已有记忆存在矛盾，返回：{{"action": "discard", "reason": "contradictory"}}
3. 如果新条目包含已有记忆未覆盖的新颖、无矛盾的信息，返回：{{"action": "keep"}}

只返回合法JSON，能够被json.loads()解析。不要包含```json。
"""

CONSENSUS_FILTER_PROMPT = """你需要通过移除已被共识覆盖的信息来压缩角色的记忆。
...
## 要求：
重写角色的记忆，只保留共识中尚未包含的独特信息。移除所有冗余/共享的部分，只保留该角色独有的内容（个人观察、私人想法、独特行动、共识中未体现的具体对话）。

规则：
1. 如果这条记忆完全被共识覆盖（没有剩余独特信息），返回：{{"action": "remove"}}
2. 如果存在独特信息，返回：{{"action": "compress", "compressed": "<仅包含独特部分的重写记忆>"}}
   - 压缩版本必须明显短于原文。
   - 保留角色的视角和语气。
   - 不要包含共识已经覆盖的任何信息。
